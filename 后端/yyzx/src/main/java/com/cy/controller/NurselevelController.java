...
import com.cy.service.impl.NurselevelitemServiceImpl;
...
public class NurselevelController {
    ...
    @Operation(summary = "查询护理级别列表(根据status查询1-启用 2-停用)")
    ...
    public ResultVo removeNurseLevelItem(@PathVariable Integer levelId,@PathVariable Integer itemId){
        ...
        lqw.eq(Nurselevelitem::getLevelId,levelId).eq(Nurselevelitem::getItemId,itemId);
    ...
    public ResultVo addItemToLevel(@RequestBody Nurselevelitem nurselevelitem){
        ...
        if(count>0){
            return ResultVo.fail("当前护理级别已存在相同的护理项目");
        }
