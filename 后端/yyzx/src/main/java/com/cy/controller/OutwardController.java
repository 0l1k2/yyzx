...
public class OutwardController {
    @Autowired
    private IOutwardService outerwardService;

   @Operation(summary = "根据用户id查询对应的客户信息")
   @PostMapping("/selectOuterwardVo")
    public ResultVo selectOuterwardVo(@RequestBody OutwardDto outerwardDto) {
        return outerwardService.selectOutwardVo(outerwardDto);
    }
    @Operation(summary = "外出审批")
    @PostMapping("/examineOutward")
    public ResultVo examineOutward(@RequestBody Outward outward) {
        return outerwardService.examineOutward(outward);
    }
    @Operation(summary = "添加外出审批")
    @PostMapping("/addOutward")
    public ResultVo addOutward(@RequestBody Outward outward) {
        return outerwardService.save(outward)?ResultVo.ok("添加成功"):ResultVo.fail("添加失败");
    }
    @Operation(summary = "撤回外出审批")
    @GetMapping("/delOutward/{id}")
    public ResultVo delOutward(@PathVariable Integer id){
        return outerwardService.removeById(id)?ResultVo.ok("撤回成功"):ResultVo.fail("撤回失败");
    }
    ...
    public ResultVo updateBackTime(@RequestBody Outward outward) {
        ...
        return outerwardService.update(outward, luw)?ResultVo.ok("登记成功"):ResultVo.fail("登记失败");
    }
