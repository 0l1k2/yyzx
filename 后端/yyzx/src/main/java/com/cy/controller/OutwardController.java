...
public class OutwardController {
    ...
    public ResultVo addOutward(@RequestBody Outward outward){
        return outwardService.save(outward)?ResultVo.ok("添加外出审批成功"):ResultVo.fail("添加外出审批失败");
    }

    @Operation(summary = "撤回外出审批")
    @GetMapping("/delOutward/{id}")
    public ResultVo delOutward(@PathVariable Integer id){
        return outwardService.removeById(id)?ResultVo.ok("撤回外出审批成功"):ResultVo.fail("撤回外出审批失败");
    }
    ...
    public ResultVo updateBackTime(@RequestBody Outward outward){
        ...
        luw.eq(Outward::getId,outward.getId())
                .set(Outward::getActualreturntime,outward.getActualreturntime());
        return outwardService.update(outward,luw)?ResultVo.ok("登记回院时间成功"):ResultVo.fail("登记回院时间失败");
    }
