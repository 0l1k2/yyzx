          ...
          <el-table :data="userList" style="width: 100% ;color:black;" stripe>
            ...
            <el-table-column align="center" label="状态" width="120">
              <template #default="scope">
                {{ Number(scope.row.isDeleted) === 1 ? '停用' : '启用' }}
              </template>
            </el-table-column>
...
const queryParams = ref({
  status: "1",         // 查询默认状态1 - 启用
  username: "",
  currentPage: 1,      // 修改为currentPage，与后端保持一致
  pageSize: 6          // 与后端保持一致
});
...
const indexMethod = computed(() => {
  return (page.value.currentPage - 1) * page.value.pageSize + 1;
});
...
// 点击启用
const enable = () => {
  queryParams.value.status = "1"; // 1-启用
  queryParams.value.pageNum = 1; // 回到第一页
  getUserList();
};
...
// 点击停用
const disable = () => {
  queryParams.value.status = "2"; // 2-停用
  queryParams.value.pageNum = 1; // 回到第一页
  getUserList();
};
...
const handleCurrentChange = (curPage) => {
 ...
 queryParams.value.currentPage = curPage; // 参数pageNum是服务端接收页码参数名
  // 重新渲染表格
  getUserList();
};
...
// 点击修改
const edit = (row) => {
  dialog.value.tops = "修改用户";
  dialog.value.dialogVisible = true;
  // 初始化模态框数据
  dialog.value.item.id = row.id;
  dialog.value.item.username = row.username;
  dialog.value.item.nickname = row.nickname;
  dialog.value.item.email = row.email;
  dialog.value.item.sex = row.sex;
  dialog.value.item.isDeleted = row.isDeleted;
  dialog.value.item.phoneNumber = row.phoneNumber;
};

// 点击添加按钮
const addItem = () => {
  dialog.value.tops = "添加用户";
  dialog.value.dialogVisible = true;
  // 重置表单
  resetForm();
};
...
const save = (formName) => {
  if (itemForm.value) {
    itemForm.value.validate((valid) => {
      if (valid) {
        // 通过id判断是添加还是编辑
        if (!dialog.value.item.id) {
          dialog.value.item.createTime = getCurDate();
          const user = getSessionStorage("user");
          if (user) {
            dialog.value.item.createBy = user.id;
          }
          dialog.value.item.password = "000000";
          dialog.value.item.roleId = 2;
          
          addUser(dialog.value.item).then((res) => {
            if (res.flag) {
          ...
          updateUser(dialog.value.item).then((res) => {
            if (res.flag) {
              ElMessage.success(res.message);
              // 刷新数据表格
              getUserList();
...
const getUserList = () => {
  // 构建符合后端要求的UserDto对象
const userDto = {
  currentPage: queryParams.value.currentPage, // 使用正确的页码
  pageSize: queryParams.value.pageSize,
  nickName: queryParams.value.username
};
  
  console.log('发送请求:', userDto); // 调试日志
  
  // 检查分页参数
  if (!userDto.currentPage || !userDto.pageSize) {
    ElMessage.error('分页参数错误，请刷新页面重试');
    return;
  }
  
  findAllUser(userDto).then((res) => {
    if (res && res.data) {
      userList.value = res.data.records || [];
      page.value.total = res.data.total || 0;
      page.value.pageSize = res.data.size || page.value.pageSize;
      page.value.currentPage = res.data.current || page.value.currentPage;
      page.value.pageCount = res.data.pages || 0;
    } else {
      if (res && res.message) {
