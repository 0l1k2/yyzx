...
const enable = () => {
  btnFlag.value = true;
  queryParams.value.levelStatus = "1";
  getListNurseLevel();
};
// 停用
const disable = () => {
  btnFlag.value = false;
  queryParams.value.levelStatus = "2";
  getListNurseLevel();
};
// 关闭模态框
const handleClose = () => {
...
// 重置表单
const resetForm = () => {
  if (levelForm.value) levelForm.value.resetFields();
  dialog.value.level = {
    id: "",
    levelName: "",
    levelStatus: ""
...
const addLevel = () => {
  ...
  nextTick(() => {
    if (levelForm.value) levelForm.value.clearValidate();
  });
};
// 编辑
const edit = (row) => {
  dialog.value.tops = "信息编辑";
  dialog.value.dialogVisible = true;
  nextTick(() => {
    dialog.value.level.id = row.id;
    dialog.value.level.levelName = row.levelName;
    dialog.value.level.levelStatus = row.levelStatus;
    if (levelForm.value) levelForm.value.clearValidate();
  });
};
// 护理项目配置
const itemToLevel = (row) => {
  router.push({ path: "/nurse/addItemToLevel", query: { levelId: row.id, levelName: row.levelName } });
};
// 保存
const save = () => {
  ...
  levelForm.value.validate((valid) => {
    if (!valid) return;
    if (!dialog.value.level.id) {
      addNurseLevel(dialog.value.level).then(res => {
        if (res.flag) {
      ...
      updateNurseLevel(dialog.value.level).then(res => {
        if (res.flag) {
          ElMessage.success(res.message);
          getListNurseLevel();
          handleClose();
...
// 查询护理级别列表
const getListNurseLevel = () => {
  listNurseLevel(queryParams.value).then(res => {
