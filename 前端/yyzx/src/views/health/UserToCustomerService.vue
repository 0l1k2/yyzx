          ...
          <el-col :span="13">
            <div class="table-main">
              ...
              <div style="margin-top:15px">
                ...
                /> -->
                <el-pagination
                  :page-size="managerPage.pageSize"      
                  background
                  :current-page="managerPage.currentPag" 
                  layout="prev, pager, next"
                  :total="managerPage.total"             
                  @current-change="handleCurrentChangeManager"
                />
              </div>
            </div>
...
import { selectCustomer as getKhxxList, editCustomer } from "../../api/customerApi.js";

const route = useRoute();
const nickName = ref(route.query.nickName);

// 无管家客户数据 & 分页配置
const notManagerKhxxList = ref([]);
const page = ref({
  total: 0,
  pageSize: 6,
  currentPag: 1,
...
// 管家服务的客户数据 & 分页配置
const managerKhxxList = ref([]);
const managerPage = ref({
  total: 0,
  pageSize: 6,
  currentPag: 1,
  pagCount: 0
});
const managerQueryParams = ref({
  currentPage: 1,
  pageSize: 6,
...
const managerKhxxIndexMethod = computed(() => (index) => {
  return (managerPage.value.currentPag - 1) * managerPage.value.pageSize + index + 1;
});

// API调用方法
const getNotManagerKhxxList = async () => {
  try {
    // 确保参数正确
    const params = {
      ...
      manType: queryParams.value.manType
    };
    
    const res = await getKhxxList(params);
    
    if (res?.data) {
      notManagerKhxxList.value = res.data.records;
      page.value.total = res.data.total;
      page.value.pageSize = res.data.size;
      page.value.currentPag = res.data.current;
      page.value.pagCount = res.data.pages;
    }
  } catch (err) {
    console.error('获取无管家客户列表失败:', err);
    ElMessage.error('获取无管家客户列表失败');
  }
};

const getManagerKhxxList = async () => {
  try {
    // 确保参数正确
    const params = {
      currentPage: managerPage.value.currentPag,
      pageSize: managerPage.value.pageSize,
      userId: managerQueryParams.value.userId
    };
    ...
    if (res?.data) {
      ...
      managerPage.value.total = res.data.total;
      managerPage.value.pageSize = res.data.size;
      managerPage.value.currentPag = res.data.current;
      managerPage.value.pagCount = res.data.pages;
    }
  } catch (err) {
    console.error('获取管家客户列表失败:', err);
    ElMessage.error('获取管家客户列表失败');
  }
};

// 事件处理方法
const query = () => {
  page.value.currentPag = 1;
...
const handleCurrentChangeManager = (curPage) => {
  managerPage.value.currentPag = curPage;
  managerQueryParams.value.currentPage = curPage;
  getManagerKhxxList();
};

const changeRow = async (row) => {
  try {
    await ElMessageBox.confirm('确定将该客户分配给当前管家吗?', '提示', { type: 'warning' });
...
const del = async (id) => {
  try {
    await ElMessageBox.confirm('确定移除该客户吗?', '提示', { type: 'warning' });
