...
const save = async () => {
    try {
        // 调用API保存外出申请
        const res = await addOutwardRecord(dialogItem.value)
        
        if (res.flag) {
            ElMessage.success('外出申请保存成功')
            dialog.dialogTimeVisible.value = false
            loadOutwardRecords()
        } else {
            ElMessage.error(res.message || '保存失败')
        }
    } catch (error) {
        console.error('保存外出申请失败', error)
...
// 审批外出申请
const examine = (id) => {
    dialogTops.value = '审批外出申请'
    dialog.dialogTimeVisible.value = true
    dialogItem.value = { 
        id,
        auditstatus: 1
    }
...
// 审批保存
const examineOutward = async () => {
    try {
        const res = await auditOutwardRecord(dialogItem.value)
...
// 保存回院时间
const updateBackTime = async () => {
    try {
        const res = await updateOutwardRecord(dialogItem.value)
        
        if (res.flag) {
            ElMessage.success('回院时间已更新')
            dialog.dialogTimeVisible.value = false
            loadOutwardRecords()
        } else {
